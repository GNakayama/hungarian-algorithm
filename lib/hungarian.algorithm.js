...
=======
        ...
        while (delted[delta.s] !== undefined) {
            delta = remove(queue);
        }

        steps.push([nodes[delta.s],'#606', 3]);

        updatePrices(tree, prices, delta.weight - acc);

        if (matching[delta.s] !== undefined) {
            if (tree.length === delta.level) {
                tree.push([[delta.s, delta.level]]);
            } else {
                tree[delta.level].push([delta.s, delta.level]);
            }

            vertex =  matching[delta.s]

            var edge = e[delta.p*n + delta.s - m];
            var type = 0;
            ...
            edges.push([delta.p, delta.s]);
            steps.push([e[vertex*n + delta.s - m], '#0a0', 4]);
            edges.push([vertex, delta.s]);

            if (tree.length === delta.level + 1) {
                tree.push([[vertex, delta.level + 1]]);
            } else {
                tree[delta.level + 1].push([vertex, delta.level + 1]);
            }

            visited[delta.s] = true;
            parents[delta.s] = delta.p;
            parents[vertex] = delta.s;
            level = delta.level + 1;
        } else {
            parents[delta.s] = delta.p;
            path = buildPath(parents, delta.s);
            break;
...
/*            var edge = e[delta.p*n + delta.s - m];
            ...
            steps.push([edge, '#660', type]);
            edges.push([delta.p, delta.s]);
            steps.push([nodes[delta.s],'#660', 3]);
            break;*/
        }
        ...
        delted[delta.s] = true;
        delta = delta.weight - acc;
        acc += delta;

        son = bdf(adjs, matching, prices, tree, visited, path, vertex, parents, edges, minEdges, queue, acc, level);

        if (son >= 0) {
            steps.push([nodes[queue[0].s],'#006', 3]);
            steps.push([nodes[son],'#606', 3]);
            steps.push([nodes[son],'#006', 3]);
        }

        path = buildPath(parents, son);
>>>>>>> Stashed changes
...
=======
                    var head1 = queue[0];
                    insert(queue, {'weight': cost + acc, 'p': v, 's': adj[2], 'level': level + 1});
                    var head2 = queue[0];

                    if (head1 !== head2) {
                        steps.push([nodes[head2.s],'#060', 3]);
>>>>>>> Stashed changes
